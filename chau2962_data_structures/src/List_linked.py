...
class List:
    ...
    def _append_list(self, target):
        """
        ---------------------------------------------------------
        Appends target list to self.
        Use: source.clear()
        -------------------------------------------------------
        Parameters:
            target - Target list to append (List of ?)
        Returns:
            None
        -------------------------------------------------------
        """
    ...
    def __eq__(self, target):
        ...
        h1 = self._front
        h2 = target._front
        while (h1 is not None or h2 is not None) and equals is True:
            if (h1 is not None and h2 is None) or (h1 is None and h2 is not None):
                equals = False  # unequal lengths
            elif h1._value != h2._value:
                equals = False
            else:
                h1 = h1._next
                h2 = h2._next
    ...
    def split(self):
        ...
        target1 = List()
        target2 = List()
        mid = (self._count // 2 + self._count % 2) - 1
        db = False
        i = 0
