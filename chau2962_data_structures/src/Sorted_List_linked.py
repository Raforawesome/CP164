...
class Sorted_List:
    ...
    def combine(self, source1, source2):
        ...
        while source1._front is not None or source2._front is not None:
            ...
            if s1 is not None and s2 is not None:
                max = s2._value > s1._value and source2 or source1
                min = s2._value < s1._value and source2 or source1
                self._grab_front(min)
                self._grab_front(max)
            elif s1 is not None:
                self._grab_front(source1)
            else:
                self._grab_front(source2)
        return None

    def _grab_rear(self, target):
        node = target._rear
        prev = None
        temp = target._front
        while prev is None:
        ...
        if self._front is None:
            ...
            current = self._front
            while current._value <= node._value:
                previous = current
                current = current._next
            previous._next = node
        self._count += 1
    ...
    def _grab_front(self, target):
        node = target._front
        target._front = node._next
